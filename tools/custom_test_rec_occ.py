...
from projects.mmdet3d_plugin.OccWorld.apis.test import custom_multi_gpu_test
...
import numpy as np
from projects.mmdet3d_plugin.datasets.OccWorld import get_nuScenes_label_name
from projects.mmdet3d_plugin.OccWorld.utils.metric_util import MeanIoU, multi_step_MeanIou
...
def main():
    ...
    if not distributed:
        ...
        outputs = custom_multi_gpu_test(model, data_loader, args.tmpdir,
                                        args.gpu_collect, CalMeanIou_sem, CalMeanIou_vox)
    ...
    if rank == 0:
        ...
        #logger.info(f'PlanRegLoss is {plan_loss/len(val_dataset_loader)}')
        print(f'Current val iou is {val_iou} while the best val iou is {best_val_iou}')
        print(f'Current val miou is {val_miou} while the best val miou is {best_val_miou}')
