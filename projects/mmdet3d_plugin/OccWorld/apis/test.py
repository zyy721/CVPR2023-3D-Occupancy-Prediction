...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False, CalMeanIou_sem=None, CalMeanIou_vox=None):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            target_occs = data['voxel_semantics'][0].cuda().unsqueeze(1)
            target_occs_iou = deepcopy(target_occs)
            target_occs_iou[target_occs_iou != 17] = 1
            target_occs_iou[target_occs_iou == 17] = 0

            CalMeanIou_sem._after_step(result['sem_pred'], target_occs)
            CalMeanIou_vox._after_step(result['iou_pred'], target_occs_iou)

            result = result['sem_pred']
